...
public class CustomerDaoImpl implements CustomerDao {
	...
	@Override
	public String updateDetails(String c_Mob_No, String c_Address, String c_Password, int c_Acc_No)
		...
		try (Connection conn = DBUtil.provideConnection()) {
			...
			PreparedStatement ps = conn
					.prepareStatement("UPDATE customer SET c_Mob_No= ?,c_Address=?,c_Password=? where c_Acc_No=?");
	...
	public String removeAccByAccNo(int c_Acc_no) throws CustomerException {
		...
		try (Connection conn = DBUtil.provideConnection()) {
			...
			PreparedStatement ps = conn.prepareStatement("DELETE FROM CUSTOMER WHERE c_Acc_No=?");
	...
	public Customer viewCustomer(int c_Acc_no) throws CustomerException {
		...
		try (Connection conn = DBUtil.provideConnection()) {
			...
			PreparedStatement ps = conn.prepareStatement("select*from customer where c_Acc_No=?");
					...
					if (x > 0) {
						...
						try (Connection conn2 = DBUtil.provideConnection()) {
							...
							PreparedStatement ps2 = conn2.prepareStatement(
									"update customer set c_Total_Amount=c_Total_Amount+? where c_Acc_No = ?");
	...
	public List<Transaction> viewTransactionHistory(int c_Acc_No) throws TransactionException {
		...
		try (Connection conn = DBUtil.provideConnection()) {
			...
			PreparedStatement ps = conn
					.prepareStatement("select*from Transaction where sender_Acc_No = ? OR receiver_Acc_No=?");
